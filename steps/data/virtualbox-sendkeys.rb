...
class Main
  ...
  def initialize
    ...
    OptionParser.new do |opts|
      opts.banner = "Usage: virtualbox-sendkeys [--vmid command_to_pipe_to] STRING\n" +
        "Where STRING can be 'ls<enter>ls<gt>/dev/null<enter>'"

      opts.on("--vmid VALUE",
              "Virtualbox VM ID") do |v|
        self.vmid = v
      end
  ...
  def send_keycode(keycode)
    command= "VBoxManage controlvm \"#{name}\" keyboardputscancode #{keycode}"
    system "#{command}"
  end

  def run
    puts "Typing on console: #{self.keystring.inspect}"
    keycodes = Scancode.string_to_keycode(self.keystring)
    for keycode in keycodes.split(' ') do
      case keycode
      when /^wait(\d*)$/ then sleep(1)
      else
        command= "VBoxManage controlvm \"#{vmid}\" keyboardputscancode #{keycode}"
        system "#{command}"
        sleep delay_s
      end
    end
